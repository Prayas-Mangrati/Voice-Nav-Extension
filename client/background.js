...
async function handleCommand(command, tabId) {
  ...
  // ===== OPEN (Enhanced Smart Logic) =====
  else if (lower.startsWith("open ")) {
    const siteName = lower.replace("open ", "").trim().toLowerCase();
    let url = "";
    ...
    if (domainMap[siteName]) {
      url = domainMap[siteName];
    } else if (
      siteName.includes("college") ||
      siteName.includes("university") ||
      siteName.includes("institute") ||
      siteName.includes("edu")
    ) {
      url = `https://www.${siteName.replace(/\s+/g, "")}.edu.in`;
    } else if (siteName.includes("gov") || siteName.includes("ministry")) {
      url = `https://www.${siteName.replace(/\s+/g, "")}.gov.in`;
    } else if (siteName.includes("india")) {
      url = `https://www.${siteName.replace(/\s+/g, "")}.in`;
    } else {
      url = `https://www.${siteName.replace(/\s+/g, "")}.com`;
    }
